...
class QString;
class QRegExp { std::string p_; mutable std::vector<std::string> captures_; public: explicit QRegExp(const char*p):p_(p){} explicit QRegExp(const std::string&p):p_(p){} const std::string& pattern()const{return p_;} int indexIn(const QString&text)const; QString cap(int index)const; };
...
template<class T> class QList:public std::vector<T>{public:using std::vector<T>::vector; void append(const T&v){this->push_back(v);} void replace(int i,const T&v){(*this)[i]=v;} QList& operator<<(const T&v){append(v);return *this;} QList& operator+=(const QList&o){this->insert(this->end(),o.begin(),o.end());return *this;} int count()const{return int(this->size());} bool isEmpty()const{return this->empty();} T& first(){return this->front();}const T&first()const{return this->front();} T& last(){return this->back();} T takeFirst(){T v=this->front();this->erase(this->begin());return v;} bool contains(const T&v)const{return std::find(this->begin(),this->end(),v)!=this->end();}};
class QStringList:public QList<QString>{public:using QList<QString>::QList;QStringList()=default;QStringList(const QList<QString>&v){this->assign(v.begin(),v.end());}QStringList&operator<<(const QString&v){append(v);return*this;}QString join(const QString&separator)const{QString out;for(size_t i=0;i<size();++i){if(i)out.append(separator);out.append(at(i));}return out;}};
template<class T> class QSet:public std::set<T>{public:using std::set<T>::set; bool contains(const T&v)const{return this->count(v)!=0;} QSet& operator<<(const T&v){this->insert(v);return *this;}};
...
template<class K,class V> class QHash:public std::unordered_map<K,V>{public:using Base=std::unordered_map<K,V>;using Base::Base; void insert(const K&k,const V&v){(*this)[k]=v;} bool contains(const K&k)const{return this->count(k)!=0;} V value(const K&k,const V&d=V())const{auto i=this->find(k);return i==this->end()?d:i->second;}};
...
class QObject {public:virtual ~QObject()=default;static QString tr(const char*s){return QString(s);}};
...
class QDebug {std::ostream*o_;public:explicit QDebug(std::ostream&o):o_(&o){}~QDebug(){*o_<<'\n';}QDebug& maybeSpace(){return *this;}QDebug&operator<<(QDebugBase base){if(base.hexadecimal)*o_<<std::hex;else *o_<<std::dec;return*this;}template<class T>QDebug&operator<<(const T&v){*o_<<v<<' ';return*this;}QDebug&operator<<(const QString&v){*o_<<v.c_str()<<' ';return*this;}QDebug&operator<<(const QByteArray&v){o_->write(v.data(),v.size());return*this;}};
